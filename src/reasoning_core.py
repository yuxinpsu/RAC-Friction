...
def main():
    ...
    train_space, test_space = build_hybrid_space(x_train, x_test, h_train, h_test)
    neighbors = build_neighbor_indices(train_space, test_space)
    ...
    model, history, y_mean, y_std = train_reasoner(data, neighbors)

    device = next(model.parameters()).device
    y_scaled = ((y_train - y_mean) / y_std).astype(np.float32)
    train_maker = BatchMaker(
        x_train,
        h_train,
        x_train,
        h_train,
    ...
    print("generating train reasoned memory features...", flush=True)
    train_reason = generate_reason_features(model, train_maker, np.arange(len(x_train)), y_mean, y_std)
    print("generating test reasoned memory features...", flush=True)
    test_reason = generate_reason_features(model, test_maker, np.arange(len(x_test)), y_mean, y_std)

    direct_reg = regression_metrics(y_test, test_reason[:, 0])
    direct_cls = classification_metrics(z_test, test_reason[:, 11:21].argmax(axis=1))
    np.savez_compressed(REASON_FILE, train_reason=train_reason, test_reason=test_reason)

    summary = {
        "saved_neighbors": str(NEIGHBOR_FILE),
    ...
    print("\n=== RETRIEVE_THEN_REASON_TRAIN_SUMMARY ===", flush=True)
