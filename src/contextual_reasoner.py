...
def main():
    ...
    neighbors = base_mod.build_neighbor_indices(train_space, test_space)
    data = {
        "x_train": x_train,
        "x_test": x_test,
        "train_emb": h_train,
    ...
    y_scaled = ((y_train - y_mean) / y_std).astype(np.float32)
    train_maker = base_mod.BatchMaker(
        x_train,
        h_train,
        x_train,
        h_train,
    ...
    test_maker = base_mod.BatchMaker(
        x_test,
        h_test,
        x_train,
        h_train,
    ...
    print("generating train contextual reasoned memory features...", flush=True)
    train_reason = generate_reason_features(model, train_maker, np.arange(len(x_train)), y_mean, y_std)
    print("generating test contextual reasoned memory features...", flush=True)
    ...
    summary = {
        ...
        "notes": {
            "module": "contextual analog evidence interaction",
            "context_layers": N_CONTEXT_LAYERS,
            "attention_heads": N_HEADS,
            "candidate_count": int(base_mod.K),
        },
    }
    SUMMARY_FILE.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print("\n=== CONTEXTUAL_REASONER_SUMMARY ===", flush=True)
